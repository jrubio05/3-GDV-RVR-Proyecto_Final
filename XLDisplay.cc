...
const char * XLDisplay::font =  "-unregistered-latin modern typewriter-dark-o-normal--0-0-0-0-c-0-iso8859-15";
//const char * XLDisplay::font =  "*-tip-*-r-*-*-12-*";
...
void XLDisplay::init(int32_t w, int32_t h, const std::string& t)
{
    ...
    int countRet;
    int maxNames = 10000;
    char** lista = XListFonts(xl_dpy, "*", maxNames, &countRet);
    for (int i = 0; i < countRet; i++)
        std::cout << lista[i] << '\n';
    XFreeFontNames(lista);
    */
